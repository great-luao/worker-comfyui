...
def validate_input(job_input):
    ...
    # Validate 'audio' in input (optional)
    audio = job_input.get("audio")
    if audio is not None:
        if not isinstance(audio, dict) or "name" not in audio or "data" not in audio:
            return None, "'audio' must be an object with 'name' and 'data' keys"
...
def upload_audio(audio):
    ...
    Args:
        audio (dict): Audio data with 'name' and 'data' keys
    ...
    try:
        ...
        files = {
            "audio": (name, BytesIO(audio_bytes), "audio/wav"),
            "overwrite": (None, "true"),
        }

        # Upload to ComfyUI
        response = requests.post(
            f"http://{COMFY_HOST}/upload/audio",
            files=files,
            timeout=60
        )
        response.raise_for_status()

        print(f"worker-comfyui - Successfully uploaded audio: {name}")
        return {"status": "success", "message": f"Successfully uploaded {name}"}

    except requests.Timeout:
        raise ValueError(f"Timeout uploading audio: {audio.get('name', 'unknown')}")
    except requests.RequestException as e:
        raise ValueError(f"Failed to upload audio: {e}")
    except Exception as e:
        raise ValueError(f"Unexpected error uploading audio: {e}")


def get_output_files(job_id):
    """
    Get list of output files generated by ComfyUI.

    Args:
        job_id (str): Job ID to identify output files
    ...
    # ComfyUI typically saves files with patterns like ComfyUI_00001_.png
    # We'll look for recently modified files
    try:
        # Get all files in output directory
        all_files = []
        for pattern in ["*.png", "*.jpg", "*.jpeg", "*.mp4", "*.avi", "*.mov"]:
            all_files.extend(glob.glob(f"{output_dir}/{pattern}"))
        ...
        # Return the most recent files (limit to reasonable number)
        recent_files = all_files[:10]  # Get up to 10 most recent files

        return [{"path": file, "filename": os.path.basename(file)} for file in recent_files]

    except Exception as e:
        print(f"worker-comfyui - Warning: Could not get output files: {e}")
        return []
...
def handler(job):
    """
    Handles a job using ComfyUI via websockets for status and image/video retrieval.
    ...
    # Upload input audio if provided
    if input_audio:
        try:
            upload_audio(input_audio)
        except ValueError as e:
            return {"error": str(e)}
        ...
        for node_id, node_output in outputs.items():
            if "images" in node_output:
                ...
                for image_info in node_output["images"]:
                    ...
                    # Build the full path in ComfyUI output directory
                    full_path = f"/runpod-volume/ComfyUI/output/{filename}"
                    if subfolder:
                        full_path = f"/runpod-volume/ComfyUI/output/{subfolder}/{filename}"
                    ...
                    if os.path.exists(full_path):
                        ...
                        print(f"worker-comfyui - Found output file: {full_path}")
                    else:
                        error_msg = f"Output file not found: {full_path}"
                        print(f"worker-comfyui - {error_msg}")
                        errors.append(error_msg)

            # Check for other output types (videos, etc.)
    ...
    if output_data:
        final_result["files"] = output_data
